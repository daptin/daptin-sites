<template>
  <q-layout view="hhh lpR fFf">
    ...
    >
      <q-list>
        <q-item-label header>
          <h3 style="color: white">Make</h3>
        </q-item-label>

        <q-item
          clickable
          @click.native="selectedTab = 'appSetting'"
        ...
        <q-item
          clickable
          @click.native="selectedTab = 'screens'"
        ...
        <q-item
          clickable
          @click.native="selectedTab = 'events'"
        ...
        <q-item
          clickable
          @click.native="selectedTab = 'publish'"
    ...
    <q-page-container>
          ...
          <form>
            ...
            <div class="q-pa-md">
              <q-badge
                ...
                text-color="white"
                class="q-mb-sm"
              >Primary Color: {{appLayout.style.primary}}</q-badge>
            ...
            <div class="q-pa-md">
              <q-badge
                ...
                text-color="white"
                class="q-mb-sm"
              >Secondary Color: {{appLayout.style.secondary}}</q-badge>
            ...
            <q-btn
              ...
              color="primary"
              @click="saveAndChangeTab('appSetting2')"
            >Next</q-btn>
          </form>
        </div>
      </div>
      ...
      <div
        style="height: 100vh"
        class="row"
        v-if="selectedTab == 'screens' || selectedTab == 'events'"
      >
        <div class="col-1">
          <div
            class="row"
            style="text-align: center; overflow-y: scroll; height: 100vh; overflow-x: hidden"
          >
            <div
              class="col-12"
              style="text-align: left; padding-left: 20px; padding-top: 20px; height: 50px"
            >
              <h3 style="color: black">Tabs</h3>
            </div>

            <div
              class="col-12 tab-icon"
              style=" height: 110px"
              @click="setTab('home')"
              :key="-1"
              clickable
            ...
            <template v-for="(menuItem, index) in appLayout.tabs">
              <div
                class="col-12 tab-icon"
                style="padding-top: 20px; height: 150px"
                @click="setTab(menuItem)"
                :key="index"
                clickable
                v-ripple
              >
                <q-icon
                  @click.stop="deleteTab(menuItem)"
                  name="delete"
                  class="tab-delete"
                  size="sm"
                  style="width: 100%; font-size: 20px; left: 30px; opacity: 0.1"
                ></q-icon>
        ...
        <div class="col-11" v-for="(screen, i) in screens" :key="i">
          <div style=" height: 100%" class="row">
            <div class="col-8" style="background: #ebf1f4;padding-top: 150px; padding-left: 250px">
              ...
              <br />
              <iframe
                style="width: 100%; max-width: 320px; min-height: 500px; z-index: -1"
            ...
            <div class="col-3" v-if="selectedTab == 'screens'">
              ...
              <div class="row">
                <div class="col-12" style="padding: 10px;">Tab Name</div>
                ...
                <div class="col-3" style="margin: 10px;">
                  <q-icon
                    ...
                    :name="tab.icon"
                    @click.native="iconSelection = true"
                  />
                </div>
                ...
                <div class="col-12" style="margin: 10px;">
                  <q-select
                    @input="updateScreenTemplate(screen, i)"
                    filled
                    map-options
                    emit-value
                    size="sm"
                ...
                <div class="col-12" style="margin: 10px;">
                  <q-select
                    ...
                    map-options
                    emit-value
                    :options="appLayout.templates"
                    option-label="name"
                    option-value="name"
                    v-model="screen.layout.template"
                  ></q-select>
                </div>
                ...
                <div class="col-12" style="margin: 10px">
                  <div class="col-3" v-if="showMapping">
                    ...
                    <table
                      style="width: 100%;"
                      v-if="screen.table && templateMap[screen.layout.template]"
                    >
                      <tbody>
                        <tr v-for="key in templateMap[screen.layout.template].variables" :key="key">
                          ...
                          <td style="padding: 0">
                            <v-select
                              :reduce="col => col.ColumnName"
                              v-model="screen.layout.transform.item[key]"
                              :options="screenColumns"
                              value="ColumnName"
                              label="ColumnName"
                            ></v-select>
                          </td>
            ...
            >
              ...
              <div
                class="row"
                style="background: #fff2ef; border-bottom: 1px solid black; padding: 10px"
                :key="actionName"
              ...
              >
                ...
                <div class="col-12" style="padding: 10px" v-if="screen.layout.actions[actionName]">
                  <v-select
                    ...
                    v-model="screen.layout.actions[actionName].params.path"
                  ></v-select>

                  <v-select
                    v-if="screen.layout.actions[actionName].type == 'action'"
                    :options="serverActions"
                    value="action_name"
                    label="label"
                    v-model="screen.layout.actions[actionName].params.action_name"
                    :reduce="val => val.action_name"
                  ></v-select>

                  <v-select
                    v-if="['post', 'put', 'delete'].indexOf(screen.layout.actions[actionName].type) > -1"
                    :options="models"
                    value="table_name"
                    label="table_name"
                ...
                <div
                  ...
                  v-if="screen.layout.actions[actionName].type == 'relocate'"
                >
                  <q-btn label="Edit this page" @click="setScreenByAction(actionName)"></q-btn>
                  <q-btn
                    label="New page"
                    @click="newScreenNameDialog = true"
                    v-if="screen.layout.actions[actionName].type == 'relocate'"
                  ></q-btn>
                </div>
                <div class="col-12" style="margin: 10px;">
      ...
      <q-card style="min-width: 400px">
        ...
        <q-card-actions align="right" class="text-primary">
          ...
          <q-btn
            flat
            label="Add"
            @click="addTab(newTabName, newTabIcon, newTabLayout)"
            v-close-popup
          />
        </q-card-actions>
      </q-card>
        ...
        <q-card-section>
          <div class="col-12">
            <q-pagination
              ...
              :max-pages="6"
              :max="parseInt((icons.length / iconsPerPage)) + 1"
            ></q-pagination>
          </div>
        </q-card-section>
      </q-card>
      ...
      <q-card style="min-width: 400px">
        ...
        <q-card-section>
          ...
          <q-select
            ...
            :options="['relocate', 'post', 'put', 'delete', 'action']"
            label="Action Type"
            autofocus
            @keyup.enter="prompt = false"
          />
    ...
    <q-dialog v-model="deleteTabDialog" persistent>
      <q-card>
        ...
        <q-card-section>Are you sure, this cannot be undone</q-card-section>
        <q-card-actions>
          <q-btn flat label="Cancel" v-close-popup />
...
export default {
  watch: {
    "appLayout.templates": function() {
      ...
      for (var i = 0; i < this.screens.length; i++) {
        this.screens[i].template = this.templateMap[
          this.screens[i].layout.template
        ];
      }
    },
  ...
  computed: {
    ...
    currentPageIcons: function() {
      ...
      if (
        ...
        this.iconSearchText.trim().length == 0
      ) {
        return this.icons.slice(
          (this.currentIconPage - 1) * this.iconsPerPage,
          this.currentIconPage * this.iconsPerPage
        );
      } else {
        return this.icons
          .filter(function(e) {
  ...
  methods: {
    ...
    setPrimary() {
      ...
      parent.parent.document.documentElement.style.setProperty(
        "--q-color-primary",
        this.appLayout.style.primary
      );
    },
    setSecondary() {
      ...
      parent.parent.document.documentElement.style.setProperty(
        "--q-color-secondary",
        this.appLayout.style.secondary
      );
    },
    //** Theming functions/
    ...
    saveLayoutTitle(title) {
      console.log(
        ...
        this.appLayout,
        title,
        this.screens[0].layoutName
      );
      this.appLayout.layoutConfiguration[
        this.screens[0].layoutName
      ].title = title;
      ...
      this.appLayout.layoutConfiguration[layoutName] = {
        ...
        transform: {
          ...
          operate: [
            {
              run:
                "function dateParse(v){return new Date(Date.parse(v));};dateParse",
              on: "created_at"
            }
          ]
        },
    ...
    deleteTab(tabInfo, confirm) {
      if (confirm != "yes") {
        ...
        for (var i = 0; i < this.appLayout.tabs.length; i++) {
          if (
            this.appLayout.tabs[i].path == this.tabToDelete.path &&
            this.appLayout.tabs[i].label == this.tabToDelete.label
          ) {
            remove = i;
            break;
          }
    ...
    updateScreenTemplate(screen) {
      ...
      for (var event in screen.template.events) {
        console.log(
          "Create event in layout",
          screen.template.events[event].name
        );
    ...
    reloadPreview() {
      document.getElementsByClassName("preview-frame")[0].src =
        document.getElementsByClassName("preview-frame")[0].src + "";
    },
